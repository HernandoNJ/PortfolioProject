...
namespace Assets.DemoProjects.GameDevHQ.FileBase.Plugins.FPS_Character_Controller
{
	...
	public class FPS_Controller : MonoBehaviour
	{
		...
		void CameraController()
		{
			...
			transform.localRotation =
					Quaternion.AngleAxis(rot.y, Vector3.up); ////rotate along the y axis by movement amount
			...
			_fpsCamera.transform.localRotation =
					Quaternion.AngleAxis(camRot.x, Vector3.right); //rotate along the x axis by movement amount
		}
		...
		void HeadBobbing()
		{
			...
			float moveH = Input.GetAxis("Horizontal"); //horizontal inputs (a, d, leftarrow, rightarrow)
			float moveV = Input.GetAxis("Vertical"); //veritical inputs (w, s, uparrow, downarrow)

			if (moveH != 0 || moveV != 0) //Are we moving?
			{
			...
			else
			{
				...
				(
						Mathf.Lerp(_fpsCamera.transform.localPosition.x, _initialCameraPos.x,
								_smooth * Time.deltaTime), //x vlaue
						Mathf.Lerp(_fpsCamera.transform.localPosition.y, _initialCameraPos.y,
								_smooth * Time.deltaTime), //y value
						0 //z value
				);
